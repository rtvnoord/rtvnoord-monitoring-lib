...
let maintenanceCache: { at: number; maintenance: boolean; message: string } | null = null
...
function shouldSkipPing(pathname: string, extra: string[]): boolean {
  const skip = [
    '/api/',
    '/_next/',
    '/logos/',
...
export async function withMonitoring(
  ...
  if (!options.skipSecurityHeaders) applySecurityHeaders(res)
  ...
  if (!toolSlug || !secret) return res
  if (shouldSkipPing(pathname, options.skipPathPrefixes || [])) return res
